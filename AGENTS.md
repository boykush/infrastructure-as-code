...
- **`terraform.yml` の mise setup は `env: false` と `add_shims_to_path: false` の両方が要る**。`mise.toml` の `[env]` は手元用で、CI に入ると `AWS_PROFILE` が OIDC で assume した認証情報を上書きし、plan が `failed to get shared config profile` で落ちる。経路が2つあるのが罠で、`env: false` が塞ぐのは `GITHUB_ENV` への export だけ——**shim は実行時に `[env]` を適用する**ので、shim を PATH から外さないと terraform に届く。tool 自体は `export_path` が raw のバイナリを PATH に置く（tfcmt が求めるのもそちら）。
